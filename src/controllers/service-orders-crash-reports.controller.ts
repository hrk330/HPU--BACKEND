...
import {ServiceOrders, CrashReports, Witness} from '../models';
import {
  ...
  @post('/serviceOrders/{id}/createCrashReport', {
    responses: {
      '200': {
        ...
        content: {
          'application/json': {schema: getModelSchemaRef(CrashReports)},
        },
      },
    },
  })
  async create(
    ...
    try {
      ...
      if (crashReport) {
        ...
        const dbCrashReport: CrashReports = await this.serviceOrdersRepository
          .crashReport(id)
          .create(crashReports);
        if (dbCrashReport) {
          if (witnessList?.length > 0) {
            for (const witness of witnessList) {
              dbCrashReport.witnessList.push(
                await this.crashReportsRepository
