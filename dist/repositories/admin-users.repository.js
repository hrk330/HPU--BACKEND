"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.AdminUsersRepository = void 0;
const tslib_1 = require("tslib");
const core_1 = require("@loopback/core");
const repository_1 = require("@loopback/repository");
const datasources_1 = require("../datasources");
const models_1 = require("../models");
let AdminUsersRepository = class AdminUsersRepository extends repository_1.DefaultCrudRepository {
    constructor(dataSource, userTasksRepositoryGetter, userCredsRepositoryGetter) {
        super(models_1.AdminUsers, dataSource);
        this.userTasksRepositoryGetter = userTasksRepositoryGetter;
        this.userCredsRepositoryGetter = userCredsRepositoryGetter;
        this.userCreds = this.createHasOneRepositoryFactoryFor('userCreds', userCredsRepositoryGetter);
        this.registerInclusionResolver('userCreds', this.userCreds.inclusionResolver);
        this.userTasks = this.createHasManyRepositoryFactoryFor('userTasks', userTasksRepositoryGetter);
        this.registerInclusionResolver('userTasks', this.userTasks.inclusionResolver);
    }
};
AdminUsersRepository = tslib_1.__decorate([
    tslib_1.__param(0, (0, core_1.inject)('datasources.MongoDb')),
    tslib_1.__param(1, repository_1.repository.getter('UserTasksRepository')),
    tslib_1.__param(2, repository_1.repository.getter('UserCredsRepository')),
    tslib_1.__metadata("design:paramtypes", [datasources_1.MongoDbDataSource, Function, Function])
], AdminUsersRepository);
exports.AdminUsersRepository = AdminUsersRepository;
//# sourceMappingURL=admin-users.repository.js.map